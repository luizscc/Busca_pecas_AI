...
// .wrangler/tmp/bundle-eNoT7j/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-eNoT7j/middleware-insertion-facade.js
init_modules_watch_stub();
...
async function engineerAgent(env, texto) {
  ...
  const contextText = Array.isArray(chunks) && chunks.length > 0 ? chunks.map((c) => `Fonte: ${c.metadata?.source || "unknown"}
...
async function ncmAgent(env, ficha, opts) {
  ...
  let contextStr = "";
  if (chunks.length > 0) {
    contextStr = "\n\nRelevant NCM documentation:\n" + chunks.map((c, i) => `[${i + 1}] ${c.content.substring(0, 500)}`).join("\n\n");
  }
  if (equivalencia) {
