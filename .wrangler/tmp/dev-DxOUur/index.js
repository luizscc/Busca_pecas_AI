...
// .wrangler/tmp/bundle-wwlnSx/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-wwlnSx/middleware-insertion-facade.js
init_modules_watch_stub();
...
init_modules_watch_stub();
async function fetchWithTimeout(url, options = {}, ms = 15e3) {
...
async function callOpenAIJSON(env, { system, user }) {
  if (!env.OPENAI_API_KEY) {
    throw new Error("OPENAI_API_KEY n\xE3o est\xE1 configurada no ambiente.");
  }
  ...
  if (!content) {
    throw new Error("Resposta da OpenAI veio sem conte\xFAdo.");
  }
  try {
...
__name(callOpenAIJSON, "callOpenAIJSON");
...
function createSupabaseClient(env) {
  ...
  if (!url || !key) {
    console.error("\u274C Credenciais Supabase n\xE3o configuradas");
    return null;
  }
  try {
...
var supabase_default = createSupabaseClient;
...
// src/lib/rag.ts
async function retrieveChunks(env, text, k = 5, documentId) {
  const supabase = supabase_default(env);
  if (!supabase) return [];
...
__name(retrieveChunks, "retrieveChunks");

// src/lib/rag-helper.ts
async function searchWithConfidence(env, query, category, topK = 5) {
  console.log(`\u{1F50D} [RAG] Buscando conhecimento interno: "${query.substring(0, 50)}..."`);
  try {
    ...
    if (category) {
      filteredChunks = chunks.filter((c) => c.category === category);
      console.log(`\u{1F4C2} [RAG] Filtrado por categoria "${category}": ${filteredChunks.length}/${chunks.length} chunks`);
    }
    const avgSimilarity = filteredChunks.reduce((sum, c) => sum + (c.similarity || 0), 0) / filteredChunks.length;
    const confidence = Math.min(avgSimilarity, 1);
    const context = filteredChunks.map((c, i) => `[Fonte ${i + 1} - Similaridade: ${(c.similarity * 100).toFixed(0)}%]
${c.content}`).join("\n\n---\n\n");
    ...
    console.log(`\u2705 [RAG] ${filteredChunks.length} chunks encontrados (confian\xE7a: ${(confidence * 100).toFixed(0)}%)`);
    return {
      context,
      confidence,
      sources,
...
__name(searchWithConfidence, "searchWithConfidence");
function enrichPrompt(basePrompt, ragResult) {
  const { context, confidence, hasContext } = ragResult;
  if (confidence >= 0.8 && hasContext) {
    console.log("\u{1F3AF} [RAG] Alta confian\xE7a - Modo RAG priorit\xE1rio");
    return {
      prompt: `${basePrompt}

CONHECIMENTO INTERNO (Alta Confian\xE7a - ${(confidence * 100).toFixed(0)}%):
${context}

INSTRU\xC7\xD5ES:
- Priorize as informa\xE7\xF5es do conhecimento interno acima
...
- Se houver conflito, prefira o conhecimento interno`,
  ...
  if (confidence >= 0.5 && hasContext) {
    console.log("\u2696\uFE0F [RAG] Confian\xE7a m\xE9dia - Modo h\xEDbrido");
    return {
      prompt: `${basePrompt}

CONHECIMENTO INTERNO (Confian\xE7a M\xE9dia - ${(confidence * 100).toFixed(0)}%):
${context}

INSTRU\xC7\xD5ES:
- Use as informa\xE7\xF5es do conhecimento interno como refer\xEAncia
- Complemente com seu conhecimento t\xE9cnico geral
- Valide e expanda as informa\xE7\xF5es quando necess\xE1rio`,
      mode: "hybrid"
    };
  }
  console.log("\u{1F916} [RAG] Baixa confian\xE7a - Modo GPT puro");
  return {
    prompt: hasContext ? `${basePrompt}

CONTEXTO DISPON\xCDVEL (Baixa Relev\xE2ncia - ${(confidence * 100).toFixed(0)}%):
${context}

INSTRU\xC7\xD5ES:
- O contexto acima tem baixa relev\xE2ncia
- Use principalmente seu conhecimento t\xE9cnico geral
- Apenas considere o contexto se for claramente aplic\xE1vel` : basePrompt,
    mode: "gpt-only"
  };
}
__name(enrichPrompt, "enrichPrompt");

// src/agents/engineer.ts
var ENGINEER_SPEC = `
Voc\xEA \xE9 um engenheiro especialista em M\xC1QUINAS PESADAS e EQUIPAMENTOS M\xD3VEIS.

Seu foco principal s\xE3o:
- Escavadeiras, p\xE1s-carregadeiras, tratores agr\xEDcolas e florestais, retroescavadeiras, motoniveladoras, caminh\xF5es fora-de-estrada,
  bombas de concreto, harvesters, forwarders e equipamentos Ponsse.

Principais fabricantes que voc\xEA conhece:
...
Tarefa:
Recebe uma descri\xE7\xE3o de pe\xE7a (texto livre) e deve responder APENAS com um JSON no formato:

{
  ...
  "oem_code": "c\xF3digo OEM principal se houver, sen\xE3o null",
  "outros_codigos": ["lista de outros c\xF3digos se existirem"],
  "descricao_tecnica": "frase t\xE9cnica curta explicando fun\xE7\xE3o da pe\xE7a",
  "specs": {
    "tipo": "pistao_axial | engrenagem | palheta | outro se aplic\xE1vel",
    "observacoes": "campo livre para detalhes relevantes (opcional)"
  }
}

Regras:
- Nunca invente c\xF3digo OEM se n\xE3o houver nenhuma indica\xE7\xE3o.
- Se tiver d\xFAvida, use "desconhecido" em fabricante ou modelo.
- A resposta DEVE ser JSON v\xE1lido conforme o formato acima, sem texto extra.
`;
async function engineerAgent(env, texto) {
  console.log("\u{1F504} [ENGINEER] Iniciando an\xE1lise t\xE9cnica...");
  ...
  if (!descricao) {
    return {
      ...
      oem_code: null,
      outros_codigos: [],
      descricao_tecnica: "Sem descri\xE7\xE3o informada.",
      specs: {}
    };
  }
  ...
  console.log(`\u{1F4CA} [ENGINEER] Modo de busca: ${mode} (confian\xE7a RAG: ${(ragResult.confidence * 100).toFixed(0)}%)`);
  const user = descricao;
  const json = await callOpenAIJSON(env, { system, user });
  const fichaSchema = external_exports.object({
    categoria: external_exports.string().nonempty().optional(),
    fabricante_maquina: external_exports.string().optional(),
    modelo_maquina: external_exports.string().optional(),
    oem_code: external_exports.string().nullable().optional(),
    outros_codigos: external_exports.array(external_exports.string()).optional(),
    descricao_tecnica: external_exports.string().optional(),
    specs: external_exports.record(external_exports.any()).optional()
  });
  const parsed = fichaSchema.safeParse(json);
  if (!parsed.success) {
    console.error("engineerAgent: resposta inv\xE1lida do modelo", parsed.error);
    return {
      categoria: "desconhecido",
      fabricante_maquina: "desconhecido",
      modelo_maquina: "desconhecido",
...
__name(engineerAgent, "engineerAgent");

// src/agents/ncm.ts
init_modules_watch_stub();
var NCM_SPEC = `
Voc\xEA \xE9 um especialista em CLASSIFICA\xC7\xC3O FISCAL (NCM) para importa\xE7\xE3o no Brasil.

Foco:
- Pe\xE7as e partes de m\xE1quinas e equipamentos dos cap\xEDtulos 84, 85 e 87,
  com \xEAnfase em m\xE1quinas pesadas, tratores, escavadeiras, bombas hidr\xE1ulicas,
  bombas de concreto (Putzmeister, Zoomlion, Schwing), componentes hidr\xE1ulicos e el\xE9tricos.

Tarefa:
Dada uma ficha t\xE9cnica de uma pe\xE7a em JSON, voc\xEA deve responder APENAS em JSON:

{
  "ncm_sugerido": "c\xF3digo NCM com 8 d\xEDgitos ou null se n\xE3o for poss\xEDvel",
  "confianca": "baixo" | "m\xE9dio" | "alto",
  "alternativas": [
    {
      "ncm": "c\xF3digo alternativo",
      "motivo": "explica\xE7\xE3o objetiva do porqu\xEA da alternativa"
    }
  ],
  "justificativa": "texto curto explicando o racioc\xEDnio da classifica\xE7\xE3o",
  "riscos_fiscais": "alertas de risco ou d\xFAvidas (opcional)"
}

Regras:
- Se a informa\xE7\xE3o da pe\xE7a for insuficiente, coloque ncm_sugerido = null e confian\xE7a = "baixo".
- Nunca invente NCM inexistente; sempre 8 d\xEDgitos.
- Se houver d\xFAvida entre duas NCMs, escolha a principal, liste as outras em 'alternativas' com motivo.
- Responda APENAS o JSON, sem texto extra.
`;
async function ncmAgent(env, ficha, opts) {
  console.log("\u{1F504} [NCM] Iniciando classifica\xE7\xE3o fiscal...");
  ...
  const ragQuery = `NCM classifica\xE7\xE3o fiscal ${ficha.categoria} ${ficha.oem_code || ""} ${ficha.descricao_tecnica || ""} cap\xEDtulo 84 85 87`;
...
Equival\xEAncia t\xE9cnica (hints):
- ${hints.join("\n- ")}`;
    }
  }
  const basePrompt = NCM_SPEC + equivalenceContext + "\nLembre-se: responda APENAS o JSON.";
  const { prompt: system, mode } = enrichPrompt(basePrompt, ragResult);
  console.log(`\u{1F4CA} [NCM] Modo de busca: ${mode} (confian\xE7a RAG: ${(ragResult.confidence * 100).toFixed(0)}%)`);
  ...
  return {
    ...
    confianca: json.confianca ?? "baixo",
...
init_modules_watch_stub();
var EQUIVALENCE_SPEC = `
Voc\xEA \xE9 um agente de EQUIVAL\xCANCIA T\xC9CNICA de PE\xC7AS para m\xE1quinas pesadas.

Entrada:
- Uma ficha t\xE9cnica em JSON gerada por um engenheiro de m\xE1quinas pesadas, com campos como:
  - categoria
  - fabricante_maquina
  - modelo_maquina
  - oem_code
...
Seu objetivo:
1) Definir um "ITEM CAN\xD4NICO" (a forma mais est\xE1vel de identificar a pe\xE7a).
2) Listar C\xD3DIGOS DE PESQUISA a serem usados em marketplaces globais (China, \xCDndia, Turquia etc.).

Voc\xEA deve responder APENAS com um JSON neste formato:

{
  "item_canonico": {
    ...
    "oem_principal": "c\xF3digo OEM principal se houver, sen\xE3o null",
    "descricao_tecnica": "descri\xE7\xE3o t\xE9cnica enxuta da pe\xE7a, pensando em busca internacional"
  },
  "codigos_pesquisa": {
    "oem_primario": ["lista com 0 ou 1 c\xF3digo OEM principal"],
    "oem_secundarios": ["lista de c\xF3digos OEM alternativos, se houver"],
    "marcas_premium": [
      {
        "marca": "Donaldson | Fleetguard | Baldwin | Mahle | Hengst | outro",
        "codigo": "c\xF3digo de pe\xE7a equivalente",
        "comentario": "porque esta pe\xE7a \xE9 considerada equivalente/premium"
      }
    ],
    "palavras_chave": [
      "termos de busca em ingl\xEAs",
      "pensando em marketplaces globais",
      "pode incluir modelo da m\xE1quina e fun\xE7\xE3o"
    ]
  }
}

Regras:
- Se n\xE3o souber o OEM principal, coloque "oem_principal": null e deixe "oem_primario": [].
- Se n\xE3o tiver equivalentes premium conhecidos, deixe "marcas_premium": [].
- N\xE3o invente c\xF3digos OEM aleat\xF3rios; use apenas padr\xF5es plaus\xEDveis.
- Sempre inclua pelo menos 2 ou 3 'palavras_chave' em ingl\xEAs para busca global.
- Responda APENAS o JSON, sem qualquer texto extra antes ou depois.
`;
var HUNTER_CHINA_SPEC = `
Voc\xEA \xE9 o agente HUNTER CHINA.

Sua miss\xE3o:
- Buscar fornecedores chineses CONFI\xC1VEIS para a pe\xE7a especificada.
- Foco em plataformas como Alibaba, 1688, Made-in-China e fornecedores diretos.

Voc\xEA deve considerar:
- Compatibilidade da pe\xE7a com a ficha t\xE9cnica (OEM, modelo, aplica\xE7\xE3o).
- Reputa\xE7\xE3o do fornecedor (anos de atividade, avalia\xE7\xF5es, transa\xE7\xF5es).
- Informa\xE7\xF5es de pre\xE7o, moeda, MOQ, condi\xE7\xF5es de envio.

Sa\xEDda esperada (por oferta):
...
- notas_compatibilidade: texto explicando por que a oferta parece compat\xEDvel.
`;
var HUNTER_INDIA_SPEC = `
Voc\xEA \xE9 o agente HUNTER INDIA.

Sua miss\xE3o:
- Buscar fornecedores na \xCDNDIA (IndiaMART, TradeIndia, exportadores diretos) para a pe\xE7a especificada.
- Seguir o mesmo padr\xE3o de sa\xEDda do Hunter China.
- Dar aten\xE7\xE3o especial a fabricantes de bombas hidr\xE1ulicas, cilindros, v\xE1lvulas e pe\xE7as de tratores (New Holland, John Deere, Mahindra, etc.).
`;
var HUNTER_TURKEY_SPEC = `
Voc\xEA \xE9 o agente HUNTER TURKEY.

Sua miss\xE3o:
- Buscar fornecedores na TURQUIA (fabricantes de bombas hidr\xE1ulicas, pe\xE7as de m\xE1quinas, bombas de concreto tipo Putzmeister/Schwing/Zoomlion).
- Foco em exportadores habituais para Europa, Am\xE9rica Latina e Oriente M\xE9dio.
- Seguir o mesmo padr\xE3o de sa\xEDda do Hunter China.
`;

// src/agents/equivalence.ts
async function equivalenceAgent(env, ficha) {
  console.log("\u{1F504} [EQUIVALENCE] Iniciando busca de equival\xEAncias...");
  const ragQuery = `${ficha.categoria} ${ficha.oem_code || ""} ${ficha.fabricante_maquina || ""} ${ficha.modelo_maquina || ""} equival\xEAncia pe\xE7as c\xF3digos OEM`;
  const ragResult = await searchWithConfidence(env, ragQuery, "equivalences", 5);
  const basePrompt = EQUIVALENCE_SPEC + "\nLembre-se: responda APENAS o JSON.";
  const { prompt: system, mode } = enrichPrompt(basePrompt, ragResult);
  console.log(`\u{1F4CA} [EQUIVALENCE] Modo de busca: ${mode} (confian\xE7a RAG: ${(ragResult.confidence * 100).toFixed(0)}%)`);
  ...
  const itemCanonico = {
    ...
    descricao_tecnica: item.descricao_tecnica ?? ficha.descricao_tecnica ?? "Componente para m\xE1quina pesada."
  };
  const codigosPesquisa = {
...
__name(equivalenceAgent, "equivalenceAgent");
...
init_modules_watch_stub();
async function buscarOfertasWeb(env, req) {
  ...
  try {
    ...
    if (!res.ok) {
      const txt = await res.text();
      console.warn(`\u26A0\uFE0F Backend de busca (${req.pais}) indispon\xEDvel:`, res.status);
      return [];
    }
    const data = await res.json();
    if (!Array.isArray(data)) {
      console.warn(`\u26A0\uFE0F Resposta inv\xE1lida do backend (${req.pais})`);
      return [];
    }
    return data;
  } catch (e) {
...
__name(buscarOfertasWeb, "buscarOfertasWeb");

// src/agents/hunter-china.ts
async function hunterChina(env, equivalencia) {
  ...
  return [
    {
      ...
      oferta: {
        url_produto: "https://alibaba.com/search?keyword=" + encodeURIComponent(desc.slice(0, 60)),
        preco_unitario_original: preco,
        moeda_original: "USD",
        moq: isFiltro ? 10 : 1,
        observacoes: `Oferta simulada coerente com item can\xF4nico: ${desc}`
      },
      notas_compatibilidade: `Mock China baseado em equival\xEAncia t\xE9cnica.`,
      qualidade_fornecedor: "media"
    }
  ];
}
__name(hunterChina, "hunterChina");

// src/agents/hunter-india.ts
init_modules_watch_stub();
async function hunterIndia(env, equivalencia) {
  ...
  return [
    {
      ...
      oferta: {
        url_produto: "https://www.indiamart.com/search.html?q=" + encodeURIComponent(desc.slice(0, 60)),
        preco_unitario_original: preco,
        moeda_original: "USD",
        moq: isFiltro ? 20 : 2,
        observacoes: `Oferta simulada coerente com item can\xF4nico: ${desc}`
      },
      notas_compatibilidade: `Mock \xCDndia baseado em equival\xEAncia t\xE9cnica.`,
      qualidade_fornecedor: "media"
    }
  ];
}
__name(hunterIndia, "hunterIndia");

// src/agents/hunter-turkey.ts
init_modules_watch_stub();
async function hunterTurkey(env, equivalencia) {
  console.log("[HUNTER-TURKEY] Spec ativo:", HUNTER_TURKEY_SPEC.slice(0, 80), "...");
  const { item_canonico, codigos_pesquisa } = equivalencia;
  const ofertasWeb = await buscarOfertasWeb(env, {
    pais: "turkey",
    item_canonico,
  ...
  const categoria = (item_canonico.categoria || "").toLowerCase();
  const isFiltro = categoria.includes("filtro") || desc.toLowerCase().includes("filter");
  const preco = isFiltro ? 70 : 1050;
  const nomeFornecedor = isFiltro ? "Istanbul Hydraulic Filters" : "Ankara Hydraulic & Concrete Parts";
  return [
    {
      fornecedor: {
        nome: nomeFornecedor,
        pais: "Turkey",
        cidade: isFiltro ? "Istanbul" : "Ankara",
        marketplace: "Direto",
        url_loja: "https://ankarahydraulic.com"
      },
      oferta: {
        url_produto: "https://ankarahydraulic.com/search?q=" + encodeURIComponent(desc.slice(0, 60)),
        preco_unitario_original: preco,
        moeda_original: "EUR",
        moq: isFiltro ? 15 : 1,
        observacoes: `Oferta simulada coerente com item can\xF4nico: ${desc}`
      },
      notas_compatibilidade: `Mock Turquia baseado em equival\xEAncia t\xE9cnica.`,
      qualidade_fornecedor: "media"
    }
  ];
}
__name(hunterTurkey, "hunterTurkey");
...
init_modules_watch_stub();
var SISTEMAS_ISO14224 = {
  HYD: "Sistema Hidr\xE1ulico",
  TRN: "Sistema de Transmiss\xE3o",
  ENG: "Sistema Motor",
  ELE: "Sistema El\xE9trico",
  FUE: "Sistema de Combust\xEDvel",
  COO: "Sistema de Arrefecimento",
  LUB: "Sistema de Lubrifica\xE7\xE3o",
  BRA: "Sistema de Freios",
  STR: "Sistema de Dire\xE7\xE3o",
  CAB: "Cabine e Controles",
  STR_SUP: "Estrutura e Suporte",
  TRA: "Sistema de Esteiras/Rodas"
};
async function classifyISO14224(env, componenteInfo) {
  const prompt = `Voc\xEA \xE9 um especialista em taxonomia ISO 14224 para equipamentos industriais e de constru\xE7\xE3o.

TAREFA: Classifique o componente abaixo de acordo com a hierarquia ISO 14224.

INFORMA\xC7\xD5ES DO COMPONENTE:
- Categoria: ${componenteInfo.categoria || "n\xE3o informado"}
- C\xF3digo OEM: ${componenteInfo.oem_code || "n\xE3o informado"}
- Fabricante do Equipamento: ${componenteInfo.fabricante_maquina || "n\xE3o informado"}
- Modelo do Equipamento: ${componenteInfo.modelo_maquina || "n\xE3o informado"}
- Descri\xE7\xE3o T\xE9cnica: ${componenteInfo.descricao_tecnica || "n\xE3o informado"}
- Descri\xE7\xE3o da Busca: ${componenteInfo.descricao_busca || "n\xE3o informado"}

SISTEMAS DISPON\xCDVEIS (c\xF3digos ISO):
${Object.entries(SISTEMAS_ISO14224).map(([cod, nome]) => `- ${cod}: ${nome}`).join("\n")}

INSTRU\xC7\xD5ES:
1. Identifique o EQUIPAMENTO principal (ex: "Escavadeira PC200-8", "Trator D6", "Caminh\xE3o 789D")
2. Identifique o SISTEMA ao qual o componente pertence (use um dos c\xF3digos acima)
3. Identifique o SUBSISTEMA/tipo de componente (ex: "Bomba", "Filtro", "Rolamento", "Veda\xE7\xE3o")
4. Gere um c\xF3digo taxonomia no formato: MODELO.SISTEMA.SUBSISTEMA
   - Use siglas curtas (max 15 chars por parte)
   - Sem espa\xE7os, use h\xEDfen se necess\xE1rio
   - Exemplo: "PC200-8.HYD.PUMP" ou "D6.TRN.BEARING"
5. Avalie sua confian\xE7a (0.0 a 1.0) na classifica\xE7\xE3o

RESPONDA APENAS COM JSON V\xC1LIDO, SEM TEXTO ADICIONAL:
{
  "equipamento": "nome completo do equipamento",
  "sistema": "c\xF3digo do sistema (ex: HYD, TRN, ENG)",
  ...
  try {
    const result = await callOpenAIJSON(env, {
      system: "Voc\xEA \xE9 um especialista em taxonomia ISO 14224 para equipamentos industriais e de constru\xE7\xE3o. Responda apenas com JSON v\xE1lido.",
      user: prompt
    });
    ...
    if (!classification.equipamento || !classification.sistema || !classification.taxonomia_iso14224) {
      throw new Error("Classifica\xE7\xE3o incompleta retornada pela IA");
    }
...
__name(classifyISO14224, "classifyISO14224");

// src/db/queries.ts
async function insertComponente(env, componente) {
  let taxonomia = null;
  try {
    ...
    console.log(`[ISO14224] Classificado: ${taxonomia.taxonomia_iso14224} (confian\xE7a: ${(taxonomia.confianca * 100).toFixed(0)}%)`);
  } catch (err) {
    console.error("[ISO14224] Erro na classifica\xE7\xE3o:", err);
  }
  ...
  if (supabase) {
    ...
    if (componente.oem_code) {
      const { data: existing } = await supabase.from("componentes").select("id").eq("oem_code", componente.oem_code).single();
      if (existing) {
        const { error: error2 } = await supabase.from("componentes").update(payload).eq("oem_code", componente.oem_code);
        if (error2) {
    ...
    const { data, error } = await supabase.from("componentes").insert([payload]).select();
    if (error) {
  ...
  console.log("\u{1F4BE} Salvando no D1 local...");
  const stmt = `
...
__name(insertComponente, "insertComponente");

// src/agents/etl.ts
async function etlAgent(env, ficha, ncm, resultados) {
  console.log("\u{1F4BE} [ETL] Preparando dados para salvar...");
  const componente = {
    ...ficha,
    ncm
  };
  console.log("\u{1F4E6} [ETL] Componente:", JSON.stringify(componente, null, 2));
  try {
...
__name(etlAgent, "etlAgent");

// src/agents/orchestrator.ts
var ORCH_CACHE = /* @__PURE__ */ new Map();
function computeConfidence(equivalencia, ncm) {
  let score = 0;
  const reasons = [];
  ...
  const primarios = Array.isArray(equivalencia?.codigos_pesquisa?.oem_primario) ? equivalencia.codigos_pesquisa.oem_primario.length : 0;
  if (primarios >= 3) {
    score += 0.35;
    reasons.push("M\xFAltiplos OEMs prim\xE1rios (>=3)");
  } else if (primarios >= 1) {
    score += 0.2;
    reasons.push("Alguns OEMs prim\xE1rios (>=1)");
  }
  const keywords = Array.isArray(equivalencia?.codigos_pesquisa?.palavras_chave) ? equivalencia.codigos_pesquisa.palavras_chave.length : 0;
  if (keywords >= 5) {
    score += 0.1;
  ...
  if (ncm?.confianca === "alto") {
    score += 0.25;
    reasons.push("NCM confian\xE7a alta");
  } else if (ncm?.confianca === "m\xE9dio") {
    score += 0.12;
    reasons.push("NCM confian\xE7a m\xE9dia");
  }
  if (score > 1) score = 1;
  return { score, reasons };
}
__name(computeConfidence, "computeConfidence");
async function orchestrator(env, input) {
  const itemDescricao = input?.descricao ?? "";
  ...
  console.log("\u{1F504} [ORCHESTRATOR] Iniciando ETL para salvar componente...");
  try {
  ...
  const payload = {
    sucesso: true,
    mensagem: "Processamento conclu\xEDdo.",
    ficha,
    equivalencia,
    ncm,
    resultados,
...
__name(orchestrator, "orchestrator");

// src/lib/document-processor.ts
init_modules_watch_stub();
function chunkText(text, maxChunkSize = 1e3) {
  const chunks = [];
  let start = 0;
...
async function processDocument(env, content, filename, category) {
  try {
    if (!env.SUPABASE) {
      return {
        success: false,
        ...
        category,
        filename,
        error: "Supabase client n\xE3o est\xE1 configurado"
      };
    }
    const documentId = Math.floor(Date.now() / 1e3);
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const { data, error } = await env.SUPABASE.rpc("insert_chunk", {
...
init_modules_watch_stub();
async function updaterAgent(env, config) {
  const result = {
  ...
  try {
    ...
    for (const comp of componentes.results) {
      try {
        ...
        for (const novoResultado of searchResult.resultados) {
          ...
          ).bind(novoFornecedor.nome || "Desconhecido", novoFornecedor.pais || "Desconhecido").first();
          if (existingSupplier) {
          ...
          const existingOffer = currentOffers.find(
            (o) => o.fornecedor_id === fornecedorId
          );
          if (existingOffer) {
            const precoNovo = novaOferta.preco_usd || null;
            const precoAtual = existingOffer.preco_usd;
            const precoNovoNum = Number(precoNovo ?? 0);
            const precoAtualNum = Number(precoAtual ?? 0);
            if (Number.isFinite(precoNovoNum) && Number.isFinite(precoAtualNum) && Math.abs(precoNovoNum - precoAtualNum) > 0.01) {
              await env.DB.prepare(
                `UPDATE ofertas 
                 SET preco_usd = ?, preco_unitario_original = ?, data_preco = datetime('now'), atualizado_em = datetime('now')
                 WHERE id = ?`
              ).bind(precoNovo, novaOferta.preco_unitario_original || precoNovo, existingOffer.id).run();
              result.ofertasAtualizadas++;
              console.log(
                `[UPDATER] Pre\xE7o atualizado: componente ${comp.id}, fornecedor ${fornecedorId}, ${precoAtual} \u2192 ${precoNovo} USD`
              );
            }
          } else {
            await env.DB.prepare(
          ...
          if (config.autoInactivate && currentOffers.length > 0) {
            ...
            for (const ofertaAntiga of currentOffers) {
              ...
              const precoAntigo = Number(ofertaAntiga.preco_usd ?? 0);
              const melhorReputacao = Number(novaReputacao) > Number(reputacaoAntiga) * (1 + Number(config.minQualityImprovement));
              const melhorPreco = Number(novoPreco) > 0 && Number(precoAntigo) > 0 && Number(novoPreco) < Number(precoAntigo) * (1 - Number(config.minPriceImprovement));
              if (config.autoInactivate && (melhorReputacao || melhorPreco) && ofertaAntiga.fornecedor_id !== fornecedorId) {
                await env.DB.prepare(
                  ...
                  melhorReputacao && melhorPreco ? "Substitu\xEDdo: melhor reputa\xE7\xE3o e pre\xE7o" : melhorReputacao ? "Substitu\xEDdo: melhor reputa\xE7\xE3o" : "Substitu\xEDdo: melhor pre\xE7o",
                  ofertaAntiga.id
                ).run();
    ...
    console.log(`[UPDATER] Conclu\xEDdo: ${result.componentesProcessados} processados, ${result.fornecedoresAdicionados} novos, ${result.fornecedoresInativados} inativados, ${result.ofertasAtualizadas} atualizados`);
  } catch (err) {
...
__name(updaterAgent, "updaterAgent");

// src/lib/job-runner.ts
async function runJob(env, jobName) {
  const startTime = (/* @__PURE__ */ new Date()).toISOString();
  const jobConfig = await env.DB.prepare(
    `SELECT * FROM jobs_config WHERE job_name = ?`
  ).bind(jobName).first();
  if (!jobConfig) {
    throw new Error(`Job n\xE3o encontrado: ${jobName}`);
  }
  const logInsert = await env.DB.prepare(
    `INSERT INTO jobs_log (job_name, started_at, status) VALUES (?, ?, 'running')`
  ).bind(jobName, startTime).run();
  const logId = logInsert.meta.last_row_id;
  try {
    let result = {};
    if (jobName === "update_suppliers") {
      const config = jobConfig.config_json ? JSON.parse(jobConfig.config_json) : { minQualityImprovement: 0.1, minPriceImprovement: 0.05, autoInactivate: true };
    ...
    const intervalDays = jobConfig.interval_days || 7;
    ...
    await env.DB.prepare(
      `UPDATE jobs_config
       ...
       WHERE job_name = ?`
    ).bind(nextRun.toISOString(), jobName).run();
    console.log(`[JOB-RUNNER] Job ${jobName} conclu\xEDdo com sucesso`);
    return result;
  } catch (err) {
    await env.DB.prepare(
      `UPDATE jobs_log
       SET finished_at = datetime('now'),
           ...
           errors_json = ?
       WHERE id = ?`
    ).bind(JSON.stringify([String(err)]), logId).run();
...
<head>
  <meta charset="UTF-8" />
  <title>Buscador de Pe\xE7as IA</title>
...
<body>
  <div class="container">
    <h1>Buscador de Pe\xE7as IA</h1>
    <p>Teste r\xE1pido do orquestrador: digite a descri\xE7\xE3o da pe\xE7a e clique em <strong>Buscar</strong>.</p>

    <!-- Bot\xF5es de navega\xE7\xE3o e configura\xE7\xE3o -->
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
      <a href="/relatorios" style="color: #22c55e; text-decoration: none; font-weight: 600; font-size: 14px;">\u{1F4CA} Ver Relat\xF3rios ISO 14224</a>
      <button type="button" class="btn-secondary" id="btnJobsConfig" style="display:inline-block">\u2699\uFE0F Atualiza\xE7\xE3o Autom\xE1tica</button>
    </div>

    <form id="form-busca">
      <label for="descricao">Descri\xE7\xE3o da pe\xE7a</label>
      <input
        id="descricao"
        type="text"
        value="bomba hidr\xE1ulica PC200-8 OEM 708-2L-00300"
        autocomplete="off"
      />
      
      <!-- Upload de planilha para busca em lote -->
      <div class="batch-upload">
        <label for="batchFile">\u{1F4C4} Busca em Lote (Opcional)</label>
        <input type="file" id="batchFile" accept=".xlsx,.xls,.csv,.txt" />
        <div class="batch-info">
          \u{1F4A1} Carregue Excel, CSV ou TXT com uma descri\xE7\xE3o por linha. O sistema processar\xE1 todas automaticamente.
        </div>
      </div>
      ...
      <div class="batch-info" style="margin-top:8px; display:flex; gap:16px; align-items:center">
        <label style="display:flex;gap:8px;align-items:center;cursor:pointer">
          ...
          <span>\u26A1 Modo r\xE1pido (pula hunters)</span>
        </label>
        <label style="display:flex;gap:8px;align-items:center;cursor:pointer">
          <input type="checkbox" id="autoToggle" />
          <span>\u{1F9E0} Auto (aprofundar se necess\xE1rio)</span>
        </label>
      </div>

      <div class="button-group">
        <button type="submit">Buscar</button>
        <button type="button" class="btn-secondary" id="btnLimpar">\u{1F5D1}\uFE0F Limpar</button>
        <button type="button" class="btn-secondary" id="btnAprofundar" style="display:none">\u{1F50E} Aprofundar</button>
      </div>
      <div class="status" id="status"></div>
    </form>

    <div class="output" id="output">{ resultado aparecer\xE1 aqui }</div>

    <!-- Se\xE7\xE3o de Upload -->
    <div class="upload-section">
      <h2>\u{1F4DA} Base de Conhecimento RAG</h2>
      <p>Adicione documentos para melhorar as respostas dos agentes. Arraste arquivos ou clique para selecionar.</p>
      
      <div class="category-tabs">
        <button class="category-tab active" data-category="ncm">\u{1F4CB} NCM</button>
        <button class="category-tab" data-category="equivalences">\u{1F504} Equival\xEAncias</button>
        <button class="category-tab" data-category="technical">\u2699\uFE0F T\xE9cnico</button>
        <button class="category-tab" data-category="suppliers">\u{1F3ED} Fornecedores</button>
      </div>

      <div class="dropzone" id="dropzone">
        <div class="dropzone-icon">\u{1F4C1}</div>
        <div class="dropzone-text">Arraste arquivos aqui ou clique para selecionar</div>
        <div class="dropzone-hint">Aceita: .txt, .md (m\xE1x 10MB por arquivo)</div>
      </div>
  ...
  <script>
    ...
    // Fun\xE7\xE3o para processar arquivo de lote
    ...
    form.addEventListener('submit', async (event) => {
      ...
      if (batchFileInput.files.length > 0) {
        ...
        try {
          ...
          statusEl.textContent = 'Conclu\xEDdo! ' + resultados.length + ' itens processados';
        } catch (err) {
      ...
      try {
        ...
        statusEl.textContent = 'Erro na requisi\xE7\xE3o';
        output.textContent = String(err);
      }
    });
    ...
    btnLimpar.addEventListener('click', () => {
      ...
      document.getElementById('output').textContent = '{ resultado aparecer\xE1 aqui }';
    ...
    // Gerenciar sele\xE7\xE3o de categoria
    ...
    // Bot\xE3o Aprofundar (roda hunters)
    ...
    async function handleFiles(files) {
      ...
      if (validFiles.length === 0) {
        showStatus('error', 'Nenhum arquivo v\xE1lido (.txt ou .md) foi selecionado.');
        return;
      }
      ...
      if (errorCount === 0) {
        showStatus('success', 
          \`\u2705 \${successCount} arquivo(s) processado(s) com sucesso! Os agentes j\xE1 podem usar esse conhecimento.\`
        );
      } else {
        showStatus('error', 
    ...
    // Carregar configura\xE7\xE3o de jobs
    async function loadJobConfig() {
      try {
        ...
        if (jobs.length > 0) {
          ...
          // Exibir \xFAltima execu\xE7\xE3o
          ...
          document.getElementById('jobLastRun').innerHTML = \`
            <strong>\xDAltima execu\xE7\xE3o:</strong> \${lastRun}<br>
            <strong>Pr\xF3xima execu\xE7\xE3o:</strong> \${nextRun}
          \`;
        }
      } catch (err) {
    ...
    async function loadJobLog() {
      try {
        ...
        if (logs.length === 0) {
          logList.innerHTML = '<div style="color:#6b7280">Nenhuma execu\xE7\xE3o registrada</div>';
          return;
        }
          ...
          return \`
            <div class="job-log-item \${statusClass}">
              ...
              <div style="margin-top:4px;font-size:11px;color:#9ca3af">
                ...
                Atualiza\xE7\xF5es: \${log.ofertas_atualizadas || 0}
              </div>
            </div>
          \`;
        }).join('');
    ...
    btnSaveJobConfig.addEventListener('click', async () => {
      try {
        ...
        if (result.success) {
          ...
          jobModalStatus.textContent = '\u2705 Configura\xE7\xE3o salva com sucesso!';
          await loadJobConfig();
        } else {
          jobModalStatus.style.background = '#7f1d1d';
          jobModalStatus.style.color = '#fca5a5';
          jobModalStatus.textContent = '\u274C Erro ao salvar configura\xE7\xE3o';
        }
    ...
    btnRunJobNow.addEventListener('click', async () => {
      try {
        ...
        if (result.success) {
          jobModalStatus.style.background = '#14532d';
          jobModalStatus.style.color = '#86efac';
          jobModalStatus.textContent = \`\u2705 Job conclu\xEDdo! Processados: \${result.result.componentesProcessados}, Novos: \${result.result.fornecedoresAdicionados}, Inativados: \${result.result.fornecedoresInativados}\`;
          await loadJobConfig();
          await loadJobLog();
        } else {
          jobModalStatus.style.background = '#7f1d1d';
...
var RELATORIOS_PAGE = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8" />
  <title>Relat\xF3rios - Taxonomia ISO 14224</title>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <style>
    body {
      font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
...
<body>
  <div class="container">
    <div class="nav">
      ...
      <a href="/relatorios">Relat\xF3rios</a>
    </div>

    <h1>\u{1F4CA} Relat\xF3rios - Taxonomia ISO 14224</h1>
    ...
    <div class="actions">
      <button id="btnFiltrar">\u{1F50D} Filtrar</button>
      <button id="btnLimparFiltros" class="btn-secondary">\u2715 Limpar Filtros</button>
      <button id="btnExportar" class="btn-secondary">\u{1F4E5} Exportar CSV</button>
    </div>
    ...
    <details open>
      ...
      <div class="column-selector" id="columnSelector">
        ...
        <label><input type="checkbox" value="taxonomia_iso14224" checked> C\xF3digo ISO 14224</label>
        ...
        <label><input type="checkbox" value="oem_code" checked> C\xF3digo OEM</label>
        <label><input type="checkbox" value="descricao_tecnica"> Descri\xE7\xE3o T\xE9cnica</label>
        <label><input type="checkbox" value="total_fornecedores" checked> Total Fornecedores</label>
        <label><input type="checkbox" value="melhor_preco_brl" checked> Melhor Pre\xE7o (BRL)</label>
        <label><input type="checkbox" value="media_reputacao"> M\xE9dia Reputa\xE7\xE3o</label>
      </div>
    </details>
  ...
  <script>
    ...
    const columnLabels = {
      ...
      taxonomia_iso14224: 'C\xF3digo ISO 14224',
      categoria: 'Categoria',
      nome_comercial: 'Nome Comercial',
      fabricante_maquina: 'Fabricante',
      modelo_maquina: 'Modelo',
      oem_code: 'C\xF3digo OEM',
      descricao_tecnica: 'Descri\xE7\xE3o T\xE9cnica',
      total_fornecedores: 'Fornecedores',
      melhor_preco_brl: 'Pre\xE7o (BRL)',
      media_reputacao: 'Reputa\xE7\xE3o'
    };
    ...
    function renderTable(data) {
      ...
      statsDiv.innerHTML = \`
        ...
        <div class="stat-card">
          <div class="stat-label">Equipamentos \xDAnicos</div>
          <div class="stat-value">\${equipamentosUnicos}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">Sistemas \xDAnicos</div>
  ...
  async fetch(req, env) {
    ...
    if (url.pathname === "/buscar-componente" && req.method === "POST") {
      try {
        ...
        return new Response(
          JSON.stringify({
            erro: "Falha ao processar requisi\xE7\xE3o",
      ...
      try {
        ...
        if (!body.content || !body.filename || !body.category) {
          return new Response(
            JSON.stringify({
              success: false,
              error: "Campos obrigat\xF3rios: content, filename, category"
            }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" }
        ...
        if (!validCategories.includes(body.category)) {
          return new Response(
            JSON.stringify({
              success: false,
              error: "Categoria inv\xE1lida. Use: " + validCategories.join(", ")
            }),
            {
              status: 400,
              headers: { "Content-Type": "application/json" }
    ...
    if (url.pathname === "/jobs-config" && req.method === "GET") {
      try {
        ...
        return new Response(
          JSON.stringify({ error: "Erro ao buscar configura\xE7\xF5es de jobs", details: String(err) }),
    ...
    if (url.pathname === "/jobs-config" && req.method === "POST") {
      try {
        ...
        if (!body.job_name) {
          return new Response(
            JSON.stringify({ error: "job_name \xE9 obrigat\xF3rio" }),
        ...
        await env.DB.prepare(
          ...
          body.enabled !== void 0 ? body.enabled : null,
          body.interval_days !== void 0 ? body.interval_days : null,
          body.config_json || null,
          body.job_name
        ).run();
        return new Response(
          JSON.stringify({ success: true, message: "Configura\xE7\xE3o atualizada" }),
          { status: 200, headers: { "Content-Type": "application/json" } }
        );
      } catch (err) {
        return new Response(
          JSON.stringify({ error: "Erro ao atualizar configura\xE7\xE3o", details: String(err) }),
          { status: 500, headers: { "Content-Type": "application/json" } }
        );
      }
    }
    if (url.pathname === "/jobs-log" && req.method === "GET") {
      try {
        ...
        return new Response(
          JSON.stringify({ error: "Erro ao buscar hist\xF3rico de jobs", details: String(err) }),
          { status: 500, headers: { "Content-Type": "application/json" } }
        );
      }
    }
    ...
    if (url.pathname === "/relatorios" && req.method === "GET") {
      ...
      try {
        ...
        const supabase = createSupabaseClient(env);
        let componentes = [];
        let filtros = { equipamentos: [], sistemas: [], subsistemas: [] };
        if (supabase) {
          console.log("\u{1F4CA} [RELATORIOS] Buscando dados do Supabase...");
          let query = supabase.from("componentes").select("id, categoria, nome_comercial, fabricante_maquina, modelo_maquina, oem_code, descricao_tecnica, equipamento, sistema, subsistema, taxonomia_iso14224");
          ...
          const { data: eqs } = await supabase.from("componentes").select("equipamento").not("equipamento", "is", null).order("equipamento");
          const { data: sists } = await supabase.from("componentes").select("sistema").not("sistema", "is", null).order("sistema");
          const { data: subs } = await supabase.from("componentes").select("subsistema").not("subsistema", "is", null).order("subsistema");
          filtros = {
            equipamentos: [...new Set((eqs || []).map((r) => r.equipamento))].map((e) => ({ equipamento: e })),
            sistemas: [...new Set((sists || []).map((r) => r.sistema))].map((s) => ({ sistema: s })),
            subsistemas: [...new Set((subs || []).map((r) => r.subsistema))].map((s) => ({ subsistema: s }))
          };
        } else {
          console.log("\u{1F4CA} [RELATORIOS] Buscando dados do D1 local...");
          let query = `
            SELECT 
              c.id,
              c.categoria,
          ...
          filtros = {
            equipamentos: equipamentos.results || [],
            sistemas: sistemas.results || [],
            subsistemas: subsistemas.results || []
          };
        }
        return new Response(
          JSON.stringify({
        ...
        return new Response(
          JSON.stringify({ error: "Erro ao buscar relat\xF3rios", details: String(err) }),
          { status: 500, headers: { "Content-Type": "application/json" } }
        );
      }
    }
    if (url.pathname === "/relatorios/export" && req.method === "GET") {
      try {
        ...
        const headers = [
          ...
          "Modelo",
          "C\xF3digo OEM",
          "Descri\xE7\xE3o T\xE9cnica",
          "Total Fornecedores",
          "Melhor Pre\xE7o (BRL)",
          "M\xE9dia Reputa\xE7\xE3o"
        ];
        ...
        return new Response(csv, {
          ...
          headers: {
            ...
            "Content-Disposition": `attachment; filename="relatorio_componentes_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv"`
          }
        });
      } catch (err) {
        return new Response(
