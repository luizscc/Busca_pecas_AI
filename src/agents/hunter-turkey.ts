// src/agents/hunter-turkey.ts
import type { Env } from "../types";
import { HUNTER_TURKEY_SPEC } from "./config";
import type { EquivalenceResult } from "./equivalence";
import { buscarOfertasWeb, WebSearchOferta } from "./web-search";

export async function hunterTurkey(
  env: Env,
  equivalencia: EquivalenceResult
): Promise<WebSearchOferta[]> {
  console.log("[HUNTER-TURKEY] Spec ativo:", HUNTER_TURKEY_SPEC.slice(0, 80), "...");

  const { item_canonico, codigos_pesquisa } = equivalencia;

  const ofertasWeb = await buscarOfertasWeb(env, {
    pais: "turkey",
    item_canonico,
    codigos_pesquisa
  });

  if (ofertasWeb.length > 0) {
    return ofertasWeb;
  }

  const desc = item_canonico.descricao_tecnica;
  const categoria = (item_canonico.categoria || "").toLowerCase();
  const isFiltro = categoria.includes("filtro") || desc.toLowerCase().includes("filter");

  const preco = isFiltro ? 70 : 1050;
  const nomeFornecedor = isFiltro
    ? "Istanbul Hydraulic Filters"
    : "Ankara Hydraulic & Concrete Parts";

  return [
    {
      fornecedor: {
        nome: nomeFornecedor,
        pais: "Turkey",
        cidade: isFiltro ? "Istanbul" : "Ankara",
        marketplace: "Direto",
        url_loja: "https://ankarahydraulic.com"
      },
      oferta: {
        url_produto:
          "https://ankarahydraulic.com/search?q=" +
          encodeURIComponent(desc.slice(0, 60)),
        preco_unitario_original: preco,
        moeda_original: "EUR",
        moq: isFiltro ? 15 : 1,
        observacoes: `Oferta simulada coerente com item canônico: ${desc}`
      },
      notas_compatibilidade: `Mock Turquia baseado em equivalência técnica.`,
      qualidade_fornecedor: "media"
    }
  ];
}


